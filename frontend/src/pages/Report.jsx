...
const REGEX = {
  iban: /\bTR\d{24}\b/gi,
  phone:
    /(\+90\s?)?0?\s?5\d{2}\s?\d{3}\s?\d{2}\s?\d{2}\b/gi,
...
const inputStyle = (ok = true) => ({
  ...
  padding: "12px 14px",
  borderRadius: 10,
  border: `1.5px solid ${ok ? "#d0d7de" : "#ef4444"}`,
  outline: "none",
  fontSize: 15,
  marginBottom: 12,
});
...
export default function Report() {
  ...
  useEffect(() => {
    const id = setTimeout(() => {
      ...
      localStorage.setItem(
        DRAFT_KEY,
        JSON.stringify({ form, filesMeta })
      );
    }, 350);
    return () => clearTimeout(id);
  }, [form, files]);

  // Yapıştırılan görsel/pdf (Ctrl/Cmd+V)
  ...
  const v = {
    ...
    victimName:
      form.reporterName.trim().length >= 3,
    victimPhone:
      form.reporterPhone
        .replace(/\D/g, "")
        .length >= 10,
  ...
  /* ===================== Files ===================== */
  const revokeAll = (arr) =>
    arr.forEach((x) => x.url && URL.revokeObjectURL(x.url));
  ...
  const onPickFiles = (list) => {
    ...
    setFiles((prev) => {
      ...
      for (const f of arr) {
        if (!f) continue;
        if (existing.has(fileKey(f))) continue;
  ...
  const handleSubmit = async () => {
    ...
    if (!canSubmit) {
      if (!v.consent) {
        setError(
          "Lütfen bilgilendirme onayını işaretleyin."
        );
        return;
      }
      setError(
        "Lütfen tüm zorunlu alanları doldurun. (İşletme, iletişim ve mağdur bilgileri)"
      );
      return;
    }
    ...
    try {
      ...
      fd.append(
        "instagramUsername",
        normIgHandle(form.instagramUsername)
      );
      fd.append(
        "instagramUrl",
      ...
      // Mağdur bilgileri (artık zorunlu)
      fd.append(
        "reporterEmail",
      ...
      if (status === 401) {
        setError(
          "Oturum doğrulaması gerekli. Lütfen e-posta doğrulamasını tamamlayın."
        );
        const target = `/verify-email?redirect=${encodeURIComponent(
          location.pathname || "/report"
        )}`;
        ...
        setError(
          "Dosya boyutu çok büyük. Lütfen 10MB altı dosyalar yükleyin."
        );
      } else if (status === 429) {
        setError(
        ...
        setError(
          "Ağ hatası. Sunucuya erişilemiyor veya CORS engeli var."
        );
      } else {
        setError(
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Lütfen yalnızca gerçeği yansıtan bilgiler
          girin. İftira, kişisel veri ihlali veya
          yasa dışı içerik paylaşımı hukuki
          sorumluluk doğurur. Delil yüklerken
          gizlemek istediğiniz veriler varsa
          önizlemede <b>“Blur”</b>’ı
          kullanabilirsiniz (önizleme içindir).
        </div>
      </div>
                  ...
                  .map((h, i) => (
                    <span
                      ...
                      onClick={() =>
                        setForm({
                          ...form,
                          instagramUsername: h.startsWith(
                            "@"
                          )
                            ? h
                            : "@" + h,
      ...
      {showPopup && (
        <div style={popupOverlay}>
          <div style={popupBox}>
            <h3>
              İhbarınız İçin Teşekkür
              Ederiz
            </h3>
            <p>
              İhbarınız için gerekli
              incelemeleri
              başlatıyoruz. <br />
              <b>
                “Duyarlı vatandaş,
